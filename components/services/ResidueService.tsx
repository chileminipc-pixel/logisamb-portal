...
export class ResidueService {
  ...
  private static getMockDataFallback<T>(endpoint: string): APIResponse<T> {
    ...
    if (endpoint.includes('/informes/facturas-impagas')) {
      ...
      const filteredFacturas = this.MOCK_FACTURAS_IMPAGAS.filter(f => {
        const belongsToCliente = f.clienteId === clienteId;
        console.log(`Factura ${f.nro_factura}: clienteId=${f.clienteId}, requested=${clienteId}, included=${belongsToCliente}`);
        return belongsToCliente;
      });
      
      console.log(`✅ Cliente ${clienteId} puede ver ${filteredFacturas.length} facturas impagas de ${this.MOCK_FACTURAS_IMPAGAS.length} totales`);
    ...
    if (endpoint.includes('/informes/guias')) {
      ...
      const filteredGuias = this.MOCK_GUIAS.filter(g => {
        ...
        console.log(`Guía ${g.guia}: clienteId=${g.clienteId}, requested=${clienteId}, included=${belongsToCliente}`);
        return belongsToCliente;
      });
  ...
  private static calculateMockStats(clienteId: number): ResidueStats {
    const guiasCliente = this.MOCK_GUIAS.filter(g => g.clienteId === clienteId);
    const facturasCliente = this.MOCK_FACTURAS_IMPAGAS.filter(f => f.clienteId === clienteId);
